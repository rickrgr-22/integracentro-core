...
// Registrar nueva evidencia y su hash SHA-256
app.post('/evidencias', async (req: Request, res: Response) => {
  try {
    ...
    // Si se proporciona contenido plano o base64, calcular el hash SHA-256
    if (contenido) {
      hash_sha256 = crypto.createHash('sha256').update(contenido).digest('hex');
    } else if (!hash_sha256 || hash_sha256.length !== 64) {
      return res.status(400).json({
        error: 'Debe proporcionar el contenido para calcular el hash o un hash_sha256 valido de 64 caracteres',
      });
    }

    // Si no envían centro_id, asociar al centro semilla por defecto
    if (!centro_id) {
      const centroDefecto = await query(
        'SELECT id FROM centros_estancia LIMIT 1;'
      );
    ...
    res.status(201).json({
      mensaje: 'Evidencia registrada y sellada criptograficamente',
...
// Listar evidencias registradas con el nombre del centro asociado
...
// Graceful Shutdown para Kubernetes
const gracefulShutdown = async (signal: string) => {
  console.log(`Recibida señal ${signal}. Cerrando servidor y pool de base de datos...`);
  server.close(async () => {
    try {
      await pool.end();
